    ...
    public LobbyScreen(final SpaceGame game){
        ...
        stage.addListener(new ClickListener() {
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                i++;
                //game.p2pConnector.sendData("Yo: " + i);


                Json json = new Json();
                JsonPayload jpl = new JsonPayload();
                jpl.setTAG("MSG");
                jpl.setValue("Aaaaaah");
                //data.setWriter(new StringWriter());
                //data.writeObjectStart();
                //data.toJson("MESSAGE", String.class);
                //data.writeValue("message", "The counter is currently at: ", String.class);
                //data.writeValue("value", i, int.class);
                //data.writeObjectEnd();
                String outString = json.toJson(jpl);
                System.out.println(outString);

                System.out.println("Message Tag: " + json.fromJson(JsonPayload.class, outString).getTAG());

                //game.p2pConnector.sendData(data);
    ...
    public void render(float delta) {
        ...
        if (onSupportedDevice){
            if (game.p2pConnector.hasConnection()){
                ...
                layout.setText(font, "Latest data: "+ latestData);
                font.draw(game.batch, layout,
                        SpaceGame.WIDTH/2 - layout.width/2, SpaceGame.HEIGHT/4 - layout.height/2
                );

                //Json data = new Json();
                //data.addClassTag("message", String.class);
                //data.toJson("yooooooooooo");
                //game.p2pConnector.sendData(data);
            } else {
                font.setColor(Color.GREEN);
